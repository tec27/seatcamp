...
export default class ChatSockets {
  ...
  setFingerprintForSocket(socket, specified) {
    const id = crypto
      .createHash('md5')
      .update(specified + this.userIdKey)
      .digest('hex')
  ...
  handleIncoming(socket, message, frames) {
    ...
    this._messageThrottle.rateLimit(this.userIdMap.get(socket), (err, limited) => {
      ...
      frameConverter(frames, message.format, this.ffmpegRunner, this.imageMagick7).then(
        video => {
    ...
    this._messageThrottle.rateLimit(userId, (err, limited) => {
      ...
      for (const f of frames) {
        if (f.type !== 'image/jpeg') {
          socket.emit('messageack', 'Invalid message: media must be of type image/jpeg', ackData)
          return
        }
      }
      frameConverter(frames, 'image/jpeg', this.ffmpegRunner, this.imageMagick7).then(
        video => {
