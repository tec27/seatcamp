...
module.exports = function(frames, format, cb) {
  ...
  function convert() {
    ...
    Object.keys(videoCodecs).forEach(function(codec) {
      outstanding++
      doFfmpeg(folder, imgExtension, videoCodecs[codec].vcodec, videoCodecs[codec].ext,
          function(err, data) {
            if (err) {
...
function doFfmpeg(folder, imgExtension, vcodecArgs, vidExtension, cb) {
  var command = 'ffmpeg -i "' + folder + '/%d' + imgExtension + '" ' +
