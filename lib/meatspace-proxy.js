...
module.exports = function(server) {
  return new MeatspaceProxy(server)
}

var socketOptions = { 'max reconnection attempts': 1000 }

inherits(MeatspaceProxy, EventEmitter)
function MeatspaceProxy(server) {
  EventEmitter.call(this)
  this.socket = ioClient(server, socketOptions)
...
MeatspaceProxy.prototype.onMessage = function(chat) {
  if (this.dropping.isDropping(chat) || this.awaiting.dec(chat.fingerprint)) {
    ...
    // This media file completed the partial collection we had, send it on!
    var complete = this.partials.remove(chat)
...
PartialMediaMap.prototype.put = function(chat) {
  ...
  return !!(this._map[chat.key].media['video/webm'] && this._map[chat.key].media['video/mp4'])
}
